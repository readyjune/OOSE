...
public class Catalogue {
    ...
    public Set<ProductGroup> getProductGroupSet(String currGroup) {
        ProductGroup pGroup = getProductGroup(currGroup);
        return pGroup.getProductGroupSet(new TreeSet<ProductGroup>());
    }
    ...
    public void readFrom(String filename) throws CatalogueException {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            addProductGroup(new ProductGroup("Products"));
            while (line != null) {
                if (line.trim().length() > 0) // Ignore blank lines
                {
                    ...
                    switch (parts.length) {
                        case 2:
                            parseProductGroup(parts[1], "Products");
    ...
    private void parseProductGroup(String groupName, String parentName) throws CatalogueException {
        ...
        ProductGroup group = groupMap.get(groupName);
        if (group == null) {
            group = new ProductGroup(groupName);
        }

        parent.addProduct(group);
        groups.add(group);
